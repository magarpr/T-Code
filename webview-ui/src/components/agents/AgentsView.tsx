...
import { DeleteAgentDialog } from "@src/components/agents/DeleteAgentDialog"
...
const AgentsView = ({ onDone }: AgentsViewProps) => {
	...
	const [currentEditingAgentSlug, setCurrentEditingAgentSlug] = useState<string | null>(null)
	...
	// Reset local name state when agent changes
	useEffect(() => {
		if (currentEditingAgentSlug && currentEditingAgentSlug !== visualMode) {
			setCurrentEditingAgentSlug(null)
			setLocalModeName("")
		}
	}, [visualMode, currentEditingAgentSlug])
	...
	// State for create agent dialog
	const [newAgentName, setNewAgentName] = useState("")
	const [newAgentSlug, setNewAgentSlug] = useState("")
	const [newAgentDescription, setNewAgentDescription] = useState("")
	const [newAgentRoleDefinition, setNewAgentRoleDefinition] = useState("")
	const [newAgentWhenToUse, setNewAgentWhenToUse] = useState("")
	const [newAgentCustomInstructions, setNewAgentCustomInstructions] = useState("")
	const [newAgentGroups, setNewAgentGroups] = useState<GroupEntry[]>(availableGroups)
	const [newAgentSource, setNewAgentSource] = useState<AgentSource>("global")
	...
	const resetFormState = useCallback(() => {
		...
		setNewAgentGroups(availableGroups)
		setNewAgentRoleDefinition("")
		setNewAgentWhenToUse("")
		setNewAgentCustomInstructions("")
		setNewAgentSource("global")
		// Reset error states
		setNameError("")
		setSlugError("")
		setDescriptionError("")
	...
	const handleNameChange = useCallback(
		(name: string) => {
			...
			setNewAgentSlug(generateSlug(name))
		},
		[generateSlug],
	)
	...
	const handleCreateAgent = useCallback(() => {
		...
		const source = newAgentSource
		const newAgent: ModeConfig = {
			slug: newAgentSlug,
			name: newAgentName,
			description: newAgentDescription.trim() || undefined,
			roleDefinition: newAgentRoleDefinition.trim(),
			whenToUse: newAgentWhenToUse.trim() || undefined,
			customInstructions: newAgentCustomInstructions.trim() || undefined,
			groups: newAgentGroups,
			source,
		}

		// Validate the agent against the schema
		const result = modeConfigSchema.safeParse(newAgent)
		...
		updateCustomMode(newAgentSlug, newAgent)
		switchMode(newAgentSlug)
		...
		newAgentSlug,
		newAgentDescription,
		newAgentRoleDefinition,
		newAgentWhenToUse, // Add whenToUse dependency
		newAgentCustomInstructions,
		newAgentGroups,
		newAgentSource,
		updateCustomMode,
	])
	...
	const openCreateAgentDialog = useCallback(() => {
		const baseNamePrefix = "New Custom Agent"
	...
	// Use a ref to store the current agentToDelete value
	const agentToDeleteRef = useRef(agentToDelete)

	// Update the ref whenever agentToDelete changes
	useEffect(() => {
		agentToDeleteRef.current = agentToDelete
	}, [agentToDelete])
	...
	useEffect(() => {
		const handler = (event: MessageEvent) => {
			...
			if (message.type === "systemPrompt") {
				...
				// Use the ref to get the current agentToDelete value
				const currentAgentToDelete = agentToDeleteRef.current
				if (message.slug && currentAgentToDelete && currentAgentToDelete.slug === message.slug) {
					setAgentToDelete({
	...
	return (
		<Tab>
					...
					<div onClick={(e) => e.stopPropagation()} className="flex justify-between items-center mb-3">
						...
						<div className="flex gap-2">
							<StandardTooltip content={t("prompts:modes.createNewMode")}>
								<Button variant="ghost" size="icon" onClick={openCreateAgentDialog}>
											...
											{modes
												...
												.map((modeConfig) => (
													<CommandItem
														...
														data-testid={`agent-option-${modeConfig.slug}`}>
							...
							<div className="flex-1">
								...
								<div className="flex gap-2">
									<Input
										type="text"
										value={
											currentEditingAgentSlug === visualMode
												? localModeName
										...
										onFocus={() => {
											...
											if (customMode) {
												setCurrentEditingAgentSlug(visualMode)
					...
					<div className="flex items-center gap-2">
						...
						{getCurrentMode() && (
							<Button
								...
								data-testid="export-agent-button">
						...
						<div className="flex-1 p-5 overflow-y-auto min-h-0">
							...
							<div className="mb-4">
								...
								<Input
									...
									value={newAgentSlug}
									onChange={(e) => {
										setNewAgentSlug(e.target.value)
									}}
									className="w-full"
								/>
							...
							<div className="mb-4">
								...
								<VSCodeRadioGroup
									...
									onChange={(e: Event | React.FormEvent<HTMLElement>) => {
										...
										setNewAgentSource(target.value as AgentSource)
									}}>
							...
							<div style={{ marginBottom: "16px" }}>
								...
								<VSCodeTextArea
									resize="vertical"
									value={newAgentRoleDefinition}
									onChange={(e) => {
										setNewAgentRoleDefinition((e.target as HTMLTextAreaElement).value)
									}}
									rows={4}
									className="w-full"
								/>
							...
							<div className="mb-4">
								...
								<VSCodeTextField
									...
									onChange={(e) => {
										setNewAgentDescription((e.target as HTMLInputElement).value)
									}}
									className="w-full"
								/>
								{descriptionError && (
							...
							<div className="mb-4">
								...
								<VSCodeTextArea
									resize="vertical"
									value={newAgentWhenToUse}
									onChange={(e) => {
										setNewAgentWhenToUse((e.target as HTMLTextAreaElement).value)
									}}
									rows={3}
									className="w-full"
								/>
							...
							<div className="mb-4">
								...
								<div className="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-2">
									{availableGroups.map((group) => (
										<VSCodeCheckbox
											key={group}
											checked={newAgentGroups.some((g) => getGroupName(g) === group)}
											onChange={(e: Event | React.FormEvent<HTMLElement>) => {
												const target =
													(e as CustomEvent)?.detail?.target || (e.target as HTMLInputElement)
												const checked = target.checked
												if (checked) {
													setNewAgentGroups([...newAgentGroups, group])
												} else {
													setNewAgentGroups(
														newAgentGroups.filter((g) => getGroupName(g) !== group),
													)
												}
											}}>
											{t(`prompts:tools.toolNames.${group}`)}
							...
							<div className="mb-4">
								...
								<VSCodeTextArea
									resize="vertical"
									value={newAgentCustomInstructions}
									onChange={(e) => {
										setNewAgentCustomInstructions((e.target as HTMLTextAreaElement).value)
									}}
									rows={4}
									className="w-full"
								/>
			...
			<DeleteAgentDialog
				...
				modeToDelete={agentToDelete}
				onConfirm={() => {
					if (agentToDelete) {
						vscode.postMessage({
							type: "deleteCustomMode",
							slug: agentToDelete.slug,
						})
