...
export async function createRun({ suite, exercises = [], systemPrompt, timeout, ...values }: CreateRun) {
	...
	try {
		...
		const cliCommand = `pnpm --filter @takara-coder/evals cli --runId ${run.id}`
		...
		const logStream = fs.createWriteStream("/tmp/takara-coder-evals.log", { flags: "a" })
