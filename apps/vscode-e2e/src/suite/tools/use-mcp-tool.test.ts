...
import { RooCodeEventName, type ClineMessage } from "@takara-coder/types"

import { waitFor, sleep } from "../utils"
import { setDefaultSuiteTimeout } from "../test-utils"

suite.skip("Takara Coder use_mcp_tool Tool", function () {
	...
	test("Should request MCP filesystem read_file tool and complete successfully", async function () {
		...
		await sleep(2000) // Wait for Takara Coder to fully initialize
		...
		try {
			...
			// Make a small modification to trigger the save event, without this Takara Coder won't load the MCP server
