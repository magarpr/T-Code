...
// Keep ExportedModeConfig as an alias for backward compatibility
type ExportedModeConfig = ExportedAgentConfig

interface ImportData {
	customModes: ExportedAgentConfig[]
}
...
export class CustomAgentsManager {
	...
	private async loadAgentsFromFile(filePath: string): Promise<AgentConfig[]> {
		try {
			...
			const result = customAgentsSettingsSchema.safeParse(settings)
	...
	private async mergeCustomAgents(projectAgents: AgentConfig[], globalAgents: AgentConfig[]): Promise<AgentConfig[]> {
		...
		// Add project agent (takes precedence)
	...
	// Keep mergeCustomModes as an alias for backward compatibility
	private async mergeCustomModes(projectModes: ModeConfig[], globalModes: ModeConfig[]): Promise<ModeConfig[]> {
		return this.mergeCustomAgents(projectModes, globalModes)
	}
	...
	private async watchCustomModesFiles(): Promise<void> {
		...
		const handleSettingsChange = async () => {
			try {
				...
				const result = customAgentsSettingsSchema.safeParse(config)
				...
				const roomodesModes = roomodesPath ? await this.loadAgentsFromFile(roomodesPath) : []
		...
		if (workspaceFolders && workspaceFolders.length > 0) {
			...
			const handleRoomodesChange = async () => {
				try {
					const settingsModes = await this.loadAgentsFromFile(settingsPath)
					const roomodesModes = await this.loadAgentsFromFile(roomodesPath)
	...
	public async getCustomAgents(): Promise<AgentConfig[]> {
		...
		// Get agents from .roomodes if it exists.
		const roomodesPath = await this.getWorkspaceRoomodes()
		const roomodesAgents = roomodesPath ? await this.loadAgentsFromFile(roomodesPath) : []

		// Create maps to store agents by source.
		const projectAgents = new Map<string, AgentConfig>()
		const globalAgents = new Map<string, AgentConfig>()

		// Add project agents (they take precedence).
		for (const agent of roomodesAgents) {
			projectAgents.set(agent.slug, { ...agent, source: "project" as const })
		}

		// Add global agents.
		for (const agent of settingsAgents) {
			if (!projectAgents.has(agent.slug)) {
				globalAgents.set(agent.slug, { ...agent, source: "global" as const })
			}
		}

		// Combine agents in the correct order: project agents first, then global agents.
		const mergedAgents = [
			...roomodesAgents.map((agent) => ({ ...agent, source: "project" as const })),
			...settingsAgents
				.filter((agent) => !projectAgents.has(agent.slug))
	...
	// Keep getCustomModes as an alias for backward compatibility
	...
	public async updateCustomAgent(slug: string, config: AgentConfig): Promise<void> {
		try {
			...
			await this.queueWrite(async () => {
				...
				await this.updateAgentsInFile(targetPath, (modes) => {
					const updatedModes = modes.filter((m: AgentConfig) => m.slug !== slug)
	...
	// Keep updateCustomMode as an alias for backward compatibility
	...
	private async refreshMergedState(): Promise<void> {
		...
		const settingsModes = await this.loadAgentsFromFile(settingsPath)
		const roomodesModes = roomodesPath ? await this.loadAgentsFromFile(roomodesPath) : []
		const mergedModes = await this.mergeCustomAgents(roomodesModes, settingsModes)
	...
	public async deleteCustomMode(slug: string, fromMarketplace = false): Promise<void> {
		try {
			...
			const projectMode = roomodesModes.find((m: AgentConfig) => m.slug === slug)
			const globalMode = settingsModes.find((m: AgentConfig) => m.slug === slug)
			...
			await this.queueWrite(async () => {
				...
				if (projectMode && roomodesPath) {
					await this.updateAgentsInFile(roomodesPath, (modes) =>
						modes.filter((m: AgentConfig) => m.slug !== slug),
					)
				}
	...
	public async importModeWithRules(
		...
		try {
			...
			for (const importMode of importData.customModes) {
				...
				const validationResult = agentConfigSchema.safeParse(modeConfig)
				if (!validationResult.success) {
					logger.error(`Invalid agent configuration for ${modeConfig.slug}`, {
				...
				await this.updateCustomAgent(importMode.slug, {
					...modeConfig,
...
// Keep CustomModesManager as an alias for backward compatibility
export { CustomAgentsManager as CustomModesManager }
